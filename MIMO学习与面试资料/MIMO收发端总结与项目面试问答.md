...
`Layer`是空间维度上的独立数据流，不是物理天线，也不是RB、子载波或OFDM符号。在同一个“OFDM符号＋子载波”时频位置上，12条Layer各提供一个复数调制符号，组成：

```text
s(k) = [L0(k), L1(k), ... , L11(k)]^T，维度为12×1
```

理论预编码关系为 `t(k)=P(k)s(k)`，其中 `P` 为 `32×12`，输出 `t(k)` 是32根发射天线各自的复数符号。因此一条Layer通常会通过权重分散到多根天线，一根天线也通常包含多条Layer的加权和。32根天线不等于32条用户数据流；本工程的独立空间流数量是12，每片FPGA的BIT部分负责其中3条。当前F0有效顶层使用 `TX_Precoding_Direct` 做固定复制映射而非真正的复数加权，这一点要和理论预编码区分开。
