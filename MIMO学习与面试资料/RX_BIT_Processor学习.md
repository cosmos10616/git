...
一句话：执行发射端 `WFRFT_TX` 的逆变换，把当前 Layer 的 4096 个复数恢复到资源映射前的符号域排列。

输入和输出都是每拍一个复数、连续 4096 拍。`alpha` 决定加权分数傅里叶变换参数；当相关配置退化到普通变换时，可把它理解为发射端频域/时域变换的逆过程。

这部分内部通常要积累完整的 4096 点块并经过 FFT 类流水线，所以 valid 不只是一个开始脉冲，而是 4096 个真实复数的有效窗口。
