...
1. 当前`TX_RRH_Chain`没有实例化IFFT或WFRFT；`TX_Unpack_Data`只改变数据排列，后面直接拆分实虚部并插入CP。因此“上游是否已经提供时域数据”或“当前版本是否旁路了IFFT”必须结合工程其他版本、文档或波形继续核实，不能仅凭信号名下结论。
...
当前代码中没有IFFT或WFRFT实例。数据经过重排后直接进入CP插入，因此此处必须把输入当作待加CP的时域采样才符合处理逻辑；但上游信号和注释又使用“子载波”描述，说明上游是否已完成时频转换、或当前版本是否缺少/旁路IFFT，仍需工程文档和波形确认。
